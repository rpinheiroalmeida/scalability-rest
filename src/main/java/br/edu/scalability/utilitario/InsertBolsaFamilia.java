...
import br.edu.scalability.Application;
	...
	public void inserir(String filename) {
		...
		try (BufferedReader reader = new BufferedReader(
			...
			while ((line = reader.readLine()) != null) {
				...
				Registro registro = new Registro(colunas.get(0), colunas.get(1), colunas.get(2), colunas.get(8),
						colunas.get(10), colunas.get(11));


				BoundStatement bstmt = pstmt.bind(UUIDs.timeBased(), registro.getUf(), registro.getCodigoMunicipio(),
						registro.getNomeMunicipio(), registro.getNomeBeneficiario(),
						Float.parseFloat(registro.getValorPago().replace(",", "")), registro.getMesAno());
